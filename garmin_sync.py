...
SPARK_CHARS = ["▂", "▄", "▆", "█"]
SPARK_SLOTS = 6   # 24h / 4h = 6 buckets
...
def get_client() -> Garmin:
    ...
    Subsequent runs: resumes from ~/.garth token — no password needed.
    GitHub Actions caches ~/.garth between workflow runs via actions/cache.
    """
...
def fetch_day(client: Garmin, day: date) -> dict:
    """
    Fetches all metrics for a single date.
    Each endpoint is individually wrapped — one flaky API call won't kill the run.
    """
    d    = day.isoformat()
    data = {"date": d}
    ...
    for act in raw_activities:
        try:
            ...
            name   = (act.get("activityName") or
                      act.get("activityType", {}).get("typeKey", "Activity")).title()
            ...
            cadence = (detail.get("averageRunningCadenceInStepsPerMinute") or
                       detail.get("averageBikingCadenceInRevPerMinute") or
                       detail.get("averageCadence"))
            if cadence:
...
def build_health_block(data: dict, client: Garmin) -> str:
    ...
    return "## Health · {d}\n\n{body}".format(d=d, body="\n\n".join(sections))


# ─── FILE WRITE ───────────────────────────────────────────────────────────────

FILE_TEMPLATE = """{health_block}
...
def write_daily_file(day: date, health_block: str):
    """Creates the daily file. Skips if it already exists."""
    ...
    path.write_text(
        FILE_TEMPLATE.format(health_block=health_block),
        encoding="utf-8"
    )
